...
<section class=" overflow-visible lg:overflow-visible mt-0 mb-0 innerpage-navbar">
    ...
    <section
        class="innerbanner  relative bg-cover bg-center h-64 md:h-96 text-white flex mt-0 items-center justify-center overflow-hidden">
        <img :src="innerpagebannerdata.community.imageUrl" alt="Banner Image"
            class="absolute inset-0 w-full h-full object-cover" />
        <div class="absolute inset-0 bg-black opacity-50"></div>
        <div class="max-w-10xl lg:mx-[4rem] px-2 lg:px-12 relative z-10 w-full">
            <h1 class=" mb-2 text-left">
                {{ innerpagebannerdata.community.title }}
            </h1>
            <p class="text-left">
                {{ innerpagebannerdata.community.subtitle }}
            </p>
        </div>
    </section>
    ...
    <!-- COUNTRY VALUE START -->
    <section
        class="country-value max-w-10xl lg:mx-[4rem] lg:px-12 flex flex-col md:flex-row items-center gap-14 py-10 px-4 md:px-16">
        <img :src="countryvalue.image" alt="countryvalue" class="rounded-xl max-w-md w-full object-cover" />
        <div>

            <h2 class="text-2xl md:text-3xl font-bold text-primarycolor" data-aos="fade-right">{{ countryvalue.title }}</h2>
            <p class="mt-4 text-gray-600" v-for="(text, idx) in countryvalue.description" :key="idx" data-aos="fade-up">
                {{ text }}
            </p>
            <a :href="countryvalue.link"
                class="innerwrapper-btn btn inline-flex items-center gap-4 !w-[180px] bg-primarycolor text-white font-semibold px-6 py-3 rounded-full transition hover:bg-[#162c72]" >
    ...
    <!-- CORPORATIVE RESPONSIBILITY START -->

    <section class="corporate-social max-w-10xl lg:mx-[4rem] lg:px-12 px-4 md:px-16 py-10">
        <div class="flex justify-between items-center mb-[5rem]">
            <h2 class=" font-extrabold text-primarycolor" data-aos="fade-right">Corporate social responsibility</h2>
            <hr class="flex-grow border-t border-blue-800 ml-4 opacity-100" />
        </div>
        <div class="grid md:grid-cols-3 gap-8" data-aos="fade-up">
            <div v-for="(card, index) in corporatecard" :key="index"
                class="bg-white rounded-xl shadow-sm overflow-hidden">
                <img :src="card.image" alt="Card Image" class=" rounded-xl max-w-md w-full object-cover" />
                <div class="card-body p-4 pl-0">
                    <h3 class="title text-textgray mb-2">{{ card.title }}</h3>
                    <p class="text-textgray">{{ card.description }}</p>
                    <a :href="card.link"
                        class="icon-hover-link inline-flex items-center mt-4 text-[#0B2B92] font-medium transition-colors duration-200 hover:text-[#081f6b]">
                        Learn more
                        <span class="icon-wrapper ml-2 p-1 rounded-full transition-colors duration-200">
    ...
    <!-- COR[PRATIVE RESPONSIBILITY END -->
...
import {
    corporatecard,
    countryvalue,
    menuItems, // <- should be a `ref` in script.js
    topbarcontactinfo,
    innerpagebannerdata,
    Getintouchinfo,
    footerData,
...
const closeDropdowns = () => {
    menuItems.value.forEach(item => {
        if (item.subItems) item.isOpen = false;
    });
};
...
// Optional: Close dropdowns on route change
