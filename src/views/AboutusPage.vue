<template>
    ...
    <section
        class="innerbanner relative bg-cover bg-center mt-0 h-64 md:h-96 text-white flex items-center justify-center overflow-hidden">
        <img :src="innerpagebannerdata.about.imageUrl" alt="Banner Image"
            class="absolute inset-0 w-full h-full object-cover" />
        <div class="absolute inset-0 bg-black opacity-50"></div>
        <div class="max-w-10xl lg:mx-[4rem] px-2 lg:px-12 relative z-10 w-full" data-aos="fade-right">
            <h1 class="mb-2 text-left">
                {{ innerpagebannerdata.about.title }}
            </h1>
            <p class="text-left">
                {{ innerpagebannerdata.about.subtitle }}
            </p>
        </div>
    </section>
    ...
    <!-- WHO WE ARE START -->
    <section class="relative z-20 mt-0 lg:-mt-20 px-4 md:px-10">
        <div
            class="bg-white rounded-xl shadow-xl p-6 md:p-10 flex flex-col md:flex-row items-center md:items-start max-w-6xl mx-auto">
            <!-- Left Content -->
            <div class="md:w-2/3 text-center md:text-left">
                <h2 class="text-xl md:text-2xl font-bold text-blue-800 mb-4" data-aos="fade-right">{{ WeAreAltasnim.heading }}</h2>
                <p class="text-textdarkgray  md:text-base leading-relaxed mb-4 text-justify">
                    {{ WeAreAltasnim.paragraph1 }}
                </p>
                <p class="text-textdarkgray md:text-base leading-relaxed mb-4 text-justify">
                    {{ WeAreAltasnim.paragraph2 }}

                </p>
                <!-- Signature Placeholder -->
                <div class="mt-6 relative bottom-0 md:pb-6">
                    <img :src="WeAreAltasnim.sign" alt="Signature" class="h-20" />

                </div>
            </div>

            <!-- Right Image and Info -->
            <div class="md:w-1/3 mt-6 md:mt-0 md:ml-8 text-center">
                <img :src="WeAreAltasnim.image" alt="Director" class="w-full max-w-xs mx-auto rounded-lg" />
                <div class="bg-orange-500 text-white py-2 px-4 mt-0 rounded-bl-md rounded-br-md">
                    <h3 class="font-bold text-textcolor">{{ WeAreAltasnim.name }}</h3>
                    <p class="text-sm text-textcolor">{{ WeAreAltasnim.title }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section
        class="country-value max-w-10xl lg:mx-[4rem] lg:px-12 flex flex-col md:flex-row items-center justify-center gap-14 py-10 px-4 md:px-16">
        <div class="md:w-1/2 w-full">
            <h2 class="text-2xl md:text-3xl font-bold text-primarycolor" data-aos="fade-right">{{ companyprofile.title }}</h2>
            <p class="mt-4 text-gray-600 text-justify" v-for="(text, idx) in companyprofile.description" :key="idx">
                {{ text }}
            </p>

            <a :href="companyprofile.link" 
                class="innerwrapper-btn btn inline-flex items-center gap-4 bg-primarycolor text-white font-semibold px-6 py-3 rounded-full transition hover:bg-primarycolor" >
                <span class="text">{{ companyprofile.linktext }}</span>
        ...
        <div class="md:w-2/5 w-full">
            <img :src="companyprofile.image" alt="companyprofile"
    ...
    <div class="max-w-10xl xl:mx-[4rem] px-2 sm:px-6 lg:px-12">
      ...
      <div class="timeline-frame">
        <div v-for="(item, index) in timelinecard" :key="index"
          :class="['frame-item slick-slide-gap', index === currentSlide ? 'is-active' : 'is-inactive']">
          <div class="relative timelinesub-card">
            <div class="timeline-sec block relative">
              <a href="#" class="block">
                ...
                <div
                  class="overlay-content absolute inset-0 rounded-lg pr-9 pt-11 text-white">
                  <h3 class="truncate font-bold text-white mb-4 text-lg"><b>{{ item.overlayTitle }}</b></h3>
            ...
            <!-- TIMELINE CONTENT SHOWN ON HOVER -->
            <div class="timeline-content">
              <h3 class=" mb-4 text-textgray">{{ item.overlayTitle }}</h3>
              <p class="text-textgray font-[400] border-l-2 border-orange-400 pl-3 mb-10 pr-3">
                {{ item.overlayparagraph }}
              </p>
              
            </div>
            <div class="view-btn absolute bottom-6 right-0 mr-6">
                <svg class="w-8 h-8 text-primarycolor" fill="none" stroke="currentColor" stroke-width="2"
                  viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" />
                </svg>
              </div>
      ...
      <!-- Timeline Nav -->
      <div class="timeline-nav max-w-10xl lg:mx-[4rem] px-2">
        <div v-for="(year, index) in timelineYears" :key="index" class="timeline-item">
    ...
    <!-- EMBRACING TECHNOLOGY START -->
    <section class="bg-gray-100 technology">
        <div class="max-w-10xl lg:mx-[4rem] px-2 lg:px-12 py-8 md:px-8">
            <div class="flex justify-between items-center mb-[5rem]">
                <h2 class="font-extrabold text-primarycolor">Embracing Technology</h2>
            ...
            <div class="flex flex-col lg:flex-row gap-8 items-stretch">
                ...
                <div class="w-full lg:w-2/3" data-aos="fade-up">
                    <ul class="pl-2 lg:pl-8">
                        <li v-for="(point, index) in technologysection.technology.filter(item => item.description)"
                            :key="index" class="mb-4 text-justify flex items-start">
                            <i
                            ...
                            <p class="text-textdarkgray text-sm md:text-base mb-0">{{ point.description }}</p>
                        </li>
                    </ul>
                </div>

                <!-- Right: Image with full height -->
                <div class="w-full lg:w-1/3 flex items-stretch">
                    <img :src="technologysection.technology.find(item => item.image)?.image" alt="Banner Image"
    ...
    <section class="bg-gray-100">
        <div class="max-w-10xl lg:mx-[4rem] px-2 lg:px-12 py-8 md:px-8">
            <div class="flex justify-between items-center mb-[5rem]">
                <h2 class="font-extrabold text-primarycolor">Health Safety and Environment</h2>
                <hr class="flex-grow border-t border-blue-800 ml-4 opacity-100" />
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-8 items-stretch">
                <div class="w-full lg:w-1/3 flex items-stretch">
                    <img :src="technologysection.healthcare.find(item => item.image)?.image" alt="Banner Image"
                        class="w-full h-full rounded-lg object-cover shadow-md" />
                </div>
                <!-- Left: List -->
                <div class="w-full lg:w-2/3" data-aos="fade-up">
                    <ul class="pl-2 lg:pl-8">
                        <li v-for="(point, index) in technologysection.healthcare.filter(item => item.description)"
                            :key="index" class="mb-4 text-justify flex items-start">
                            <i
                                class="bi bi-check2 text-secondary !border !border-secondary w-4 h-4 p-3 lg:w-8 lg:h-8 flex items-center justify-center rounded-full mr-2 text-md lg:text-lg"></i>
                            <p class="text-textdarkgray text-sm md:text-base mb-0">{{ point.description }}</p>
...
// Slick carousel initialization for main slider
...
// Slick carousel initialization for timeline
...
const resumeBannerSlider = () => startBannerAutoSlide();
...
const goToNextSlide = () => {
    $(slider.value).slick("slickNext");
};

const goToPrevSlide = () => {
    $(slider.value).slick("slickPrev");
};

// Lifecycle
onMounted(async () => {
    await nextTick();
    if (slider.value) initMainSlider();
    initTimelineSlider();
    // startBannerAutoSlide(); // Uncomment if needed
});
